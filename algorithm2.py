...
def topfiguredata(sorted_dictionary, threshold):
  ...
  T=T,
  model_id=0)
  sd_len = len(sorted_dictionary_threshold)+0.1
  H_mem_len = len(H_mem)/sd_len
  unique_H_mem = Unique(H_mem)
  unique_H_mem_len = len(unique_H_mem)/sd_len
    
  return H_mem_len, unique_H_mem_len

thresholds = [0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1, 0.0]

total_mem_acc = [len(H_mem_acc)/100]
unique_mem_acc = [len(unique_H_mem_acc)/100]
total_mem_bleu = [len(H_mem_bleu)/100]
unique_mem_bleu = [len(unique_H_mem_acc)/100]
total_mem_chrf = [len(H_mem_chrf)/100]
unique_mem_chrf = [len(unique_H_mem_acc)/100]
...
plt.ylabel('Hallucinations per Sample')
